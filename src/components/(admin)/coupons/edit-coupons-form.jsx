...
import { useEditCouponsMutation, useGetCouponsSingleQuery } from "@/service/couponsApi"

export function EditCouponsForm({ id }) {
  const router = useRouter();
  const [editCoupons, { isLoading }] = useEditCouponsMutation();
  const { data, isLoading: isLoadingCoupons } = useGetCouponsSingleQuery(id);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const response = await editCoupons({ ...values, id: data?.data?.id }).unwrap();
        console.log(response);
        if (response?.status) {
          toast.success(response?.message || "Edit Coupons successfully");
  ...
  return (
    <div className="min-h-screen flex justify-center items-center p-4">
      <Card className="w-full max-w-2xl">
        <CardHeader>
          <CardTitle className="text-xl font-bold">Edit Coupons</CardTitle>
        </CardHeader>
        <CardContent>
