...
import { useAddCarMutation } from "@/service/carApi"
import {
  Select,
  SelectContent,
  SelectItem,
...
export function AddProductsForm() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const response = await addCar({ ...values }).unwrap();
        console.log(response);
        if (response?.status) {
          toast.success(response?.message || "Add Car successfully");
  ...
  return (
    <div className="min-h-screen flex justify-center items-center p-4">
      <Card className="w-full max-w-2xl">
        <CardHeader>
          <CardTitle className="text-xl font-bold">Add Cars</CardTitle>
        </CardHeader>
        <CardContent>
