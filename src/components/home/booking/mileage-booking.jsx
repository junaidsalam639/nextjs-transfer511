...
const MileageBooking = ({ tabs }) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    dropoff_date:
      tabs === "return" && Yup.string().required("Dropoff date is required"),
    dropoff_time:
      tabs === "return" && Yup.string().required("Dropoff time is required"),
  });
      ...
      {({ values, handleChange, errors, touched }) => (
        <Form className="grid grid-cols-1 md:grid-cols-12 gap-4 mt-4">
          ...
          <div className="md:col-span-12">
            <Button
              type="submit"
              className="hover:bg-zinc-900 bg-orange-500 text-white w-full"
              disabled={isLoading}
            >
              {isLoading ? (
